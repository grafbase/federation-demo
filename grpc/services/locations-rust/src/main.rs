...
use locations::{
    ...
    HeartbeatRequest, HeartbeatResponse, Location, SearchLocationsRequest, SearchLocationsResponse,
};
...
impl LocationService for LocationsService {
                ...
                if !req.manager_name.is_empty() {
                    matches = matches
                        && loc
                            .manager_name
                            .to_lowercase()
                            .contains(&req.manager_name.to_lowercase());
                }
    ...
    type HeartbeatStream = BoxStream<'static, Result<HeartbeatResponse, Status>>;
    ...
    async fn heartbeat(
        ...
        Ok(Response::new(Box::pin(futures::stream::unfold(
            ...
            |index| async move {
                ...
                Some((Ok(HeartbeatResponse { index }), index + 1))
            },
        ))))
    }
}
